...
This project demonstrates how Splunk Enterprise and Splunk Universal Forwarder (UF) can be used to collect Windows Security Event Logs in real time and detect multiple failed login attempts that may indicate a brute-force or suspicious authentication attack. An alert is configured to trigger when a user enters the wrong password three or more times within a short time window.

This project simulates a real-world SOC (Security Operations Center) use case for monitoring authentication-related threats.
...
Download Universal Forwarder from:
...
Set the time range to Last 15 minutes or Realtime to confirm logs are coming in live.
